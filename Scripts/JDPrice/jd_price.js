...
if (url.indexOf(path2) != -1) {
    ...
    request_history_price(shareUrl, function (data) {
        if (data) {
            ...
            lowerword.data.ad.textColor = "#ffca40";
            let bestIndex = 0;
...
function lowerMsgs(data) {
    ...
    const lowerMsg = "💰历史最低价:" + `${getSpace(4)} `+ "¥" + String(lower) + `${getSpace(5)}`
    return lowerMsg
}
...
function priceSummary(data) {
    ...
    list.forEach((item, index) => {
        ...
        summary += `⚡️${item.Name}:${getSpace(5)}${item.Price}${getSpace(8)}${item.Date}${getSpace(8)}${item.Difference}\n`
    })
    return summary
}
